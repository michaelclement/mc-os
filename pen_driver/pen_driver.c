...
static int pen_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
    ...
    if ((retval = usb_register_dev(interface, &class)) < 0)
    {
        ...
        printk(KERN_ERR "\nERR - Unable to get device minor number.");
    }
    else
    {
...
/* table of devices that use this driver */
static struct usb_device_id pen_table[] = 
{
    // vendor id and product id of samsung FD
...
MODULE_DESCRIPTION("Driver for usb flash drive, based on example from sysplay.in");
